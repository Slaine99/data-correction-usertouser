...
def register():
    ...
    if request.method == "POST":
        ...
        # Create new room list for the newly registered user
        new_room = Room(id=new_user.id, chat_list=[])
        db.session.add(new_chat)
        db.session.commit()
...
def login():
    ...
    if request.method == "POST":
        ...
        if user and user.check_password(password):
            ...
            return redirect(url_for("views.chat_v2_latest"))
        else:
...
@views.route("/new-chat-v2", methods=["POST"])
@login_required
def new_chat_v2():
    user_id = session["user"]["id"]
    ...
    participant = Participant.query.filter_by(user_id=user_id, room_id=room.id).first()
    if not participant:
        # If the user is not a participant, add them to the room
        participant = Participant(user_id=user_id, room_id=room.id)
        db.session.add(participant)
        db.session.commit()

    return redirect(url_for("views.chat_v2_latest", room_id=room.id))



@views.route("/chat-v2-latest/", methods=["GET", "POST"])
@login_required
def chat_v2_latest():
    user_id = session["user"]["id"]
    ...
    rooms = Room.query.filter(Room.participants.any(user_id=user_id)).all()
    print(rooms)
    # Fetch messages for the room if it exists
    messages = ChatMessage.query.filter_by(room_id=room.id).all() if room else []

    return render_template("chat_v2latest.html",
                           room_id=room_id,
                           rooms=rooms,
                           room=room,
                           messages=messages,
